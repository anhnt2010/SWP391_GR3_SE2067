...
/**
 * Các nghiệp vụ QUẢN TRỊ tài khoản người dùng: tìm kiếm, lọc, cập nhật,
 * khóa/mở khóa, reset mật khẩu, phân quyền.
 */
public class UserAdminDAO {
    ...
    public boolean updateUser(UserView u) throws Exception {
        ...
        try (Connection conn = DBContext.getConnection();
            ...
            ps.setInt(5, u.getStoreId());

            if (u.getDepartmentId() > 0) ps.setInt(6, u.getDepartmentId());
            else ps.setNull(6, java.sql.Types.INTEGER);

            if (u.getPositionId() > 0) ps.setInt(7, u.getPositionId());
            else ps.setNull(7, java.sql.Types.INTEGER);

            if (u.getExpireAt() != null) ps.setDate(8, u.getExpireAt());
            else ps.setNull(8, java.sql.Types.DATE);

            ps.setString(9, u.getEmployeeId());
    ...
    /** Đếm số tài khoản Admin đang hoạt động — chặn việc tự khóa/hạ quyền Admin cuối cùng. */
    ...
    private UserView map(ResultSet rs) throws Exception {
        ...
        u.setStoreId(rs.getInt("store_id"));
        ...
        u.setExpireAt(rs.getDate("expire_at"));
